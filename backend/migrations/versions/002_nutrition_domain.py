...
down_revision: str | None = "001_initial_user_domain"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("idx_food_categories_name"), "food_categories", ["name"], unique=True
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_id"], ["food_categories.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["nutrition_fact_id"], ["nutrition_facts.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        op.f("idx_meals_user_logged"), "meals", ["user_id", "logged_at"], unique=False
    )
    ...
    op.create_index(
        op.f("idx_meal_items_meal"), "meal_items", ["meal_id"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.ForeignKeyConstraint(
            ["nutrition_fact_id"], ["nutrition_facts.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("idx_barcode_products_barcode"),
        "barcode_products",
        ["barcode"],
        unique=True,
    )
    ...
    op.create_index(
        op.f("idx_restaurant_menus_restaurant_item"),
        "restaurant_menus",
        ["restaurant_name", "menu_item_name"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("idx_grocery_products_name"), "grocery_products", ["name"], unique=False
    )
    ...
    op.create_index(
        op.f("idx_favorite_foods_user"), "favorite_foods", ["user_id"], unique=False
    )
    ...
    op.create_index(
        op.f("idx_recent_foods_user"), "recent_foods", ["user_id"], unique=False
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("idx_restaurant_menus_restaurant_item"), table_name="restaurant_menus"
    )
